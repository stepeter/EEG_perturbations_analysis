...
for(cond in c("allSVZ","allWVZ","pullsStn","pullsWalk")){
  for(plotBand_cort in c("theta","alpha")){
    ...
    exchangeInds = c(5,2,6,1,3,8,4,7)
    cortTheta = thetaAve[1:8,1:8]
    cortAlpha = alphaAve[1:8,1:8]
    for(i in 1:8){
      for(j in 1:8){
    ...
    png(paste(pathname,cond,"_",plotBand_cort,".png",sep=""),bg="transparent",width = 720, height = 720)
    ...
    factors = c("RS","RO","PP","LO","LS","AP","ACC","SMA")
