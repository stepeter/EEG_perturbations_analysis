function [ICA_STRUCT] = auto_frame_rej_hjh(EEG,ICA_STRUCT)
...
while ~FR.finished_frame_rej
    ...
    switch FR.method
        case 'eeg_badframes mean thres'
            FR.thres = 6; %inputdlg('iqr average:','',1,DEFAULT_MEAN_IQR_THRES,'on');
            if isempty(FR.thres)
...
%             end
        case 'eeg_badframes percentile thres'
            FR.thres = DEFAULT_PRCTILE_THRES; %inputdlg('thres,prctile:','',1,DEFAULT_PRCTILE_THRES,'on');
            if isempty(FR.thres)
...
%     if ~strcmp(FR.method,'Cancel') && ~isempty(FR.bad_frames)
        ...
        FR.bad_frame_border_dur = num2str(DEFAULT_BAD_FRAME_EVENT_BORDER_DUR); %inputdlg('Bad frame border duration:','',1,...
%             {num2str(DEFAULT_BAD_FRAME_EVENT_BORDER_DUR)},'on');
        ...
        FR.min_bad_frame_spacing = num2str(DEFAULT_MIN_SPACE_BETWEEN_BAD_FRAMES); %inputdlg('Minimum bad frame spacing:','',1,...
...
%         waitfor(h);
        ...
        reject_frame_answer='Yes'; % = questdlg('Reject selected frames?',...
