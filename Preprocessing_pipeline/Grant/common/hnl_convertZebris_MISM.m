function [finalelocs]=hnl_convertZebris_MISM(ELocs_rawfile,ELocs_file,standardcapelocs,collectionsys,numChans,ElecFolder,cerbellGroup)
...
sampleelocs='A1,A17,A23,B1,B25,B30,C12,C21,C25,D4,D12,D27';
...
elseif whatdigitized == 2 %'standard 256 '
    dig_labels = stand_labels(1:128); %these are the channel names that will be in raw zebris output file if a full set is digitized
elseif whatdigitized == 3 % 'Sample of 256'
    dig_labels=sampleelocs;
elseif whatdigitized == 4 % 'Sample of 256 + 8 externals'
    dig_labels=[sampleelocs stand_labels(129:136)];
end
...
if ~exist([ELocs_file(1:(length(ELocs_file)-4)) '_final.sfp'])
    if ~isempty(ELocs_file)==1
        ...
        %Scalp locations
        for n = 1:(length(dig_labels)-8)
            fprintf(fid,'%s\t%12.8f\t%12.8f\t%12.8f\t%i\n',dig_labels{n},elocXcoord(n),elocYcoord(n),elocZcoord(n), n);
        end
        %External electrode locations
        for p = 1:8
            n=n+1;
            fprintf(fid,'%s\t%12.8f\t%12.8f\t%12.8f\t%i\n',dig_labels{n},elocXcoord(end-8+p),elocYcoord(end-8+p),elocZcoord(end-8+p), n);
        end
        fclose(fid);
    ...
    plo=1;
    if plo ==1
        digHCS=[elocXcoord(1:length(dig_labels)-8) elocYcoord(1:length(dig_labels)-8) elocZcoord(1:length(dig_labels)-8); ...
            elocXcoord(end-7:end) elocYcoord(end-7:end) elocZcoord(end-7:end)];
        ...
        saveas(gcf,[ElecFolder 'DigitizedElecLocs' cerbellGroup],'fig');
    end
