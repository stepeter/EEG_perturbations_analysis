...
function [handle,Zi,grid,Xi,Yi] = mod_topoplot_ranks(Values,loc_file,varargin)
    ...
    for i = 1:2:length(varargin)
        ...
        switch Param
            ...
            case 'colormap'
                ...
                colormap(Value)
            %added cases for 'weights' and 'EEGdata' - Aug 21, 2015 SP
            case 'weights'
                if size(Value,2)~=4
                    error('Colormap must be a 1 x 4 matrix')
                end
                weights=Value;
            case 'eegdata'
                EEGdata=Value;
...
%
  ...
  if strcmp(ELECTRODES,'labelpoint') |  strcmp(ELECTRODES,'numpoint')
    ...
    if(isempty(titles))
        ...
        tl = title(titles); %added title parameter - Aug 21, 2015 SP
    end
...
elseif strcmp(ELECTRODES,'labelpoint') 
  scores=percentRankChans(EEGdata,weights); 
  EMARKERSIZE=20;
  if isempty(EMARKER2CHANS)
    %Modified to scatter3 for color changes - SP Aug 21, 2015
    hp2 = scatter3(y,x,ones(size(x))*ELECTRODE_HEIGHT,...
        EMARKERSIZE, scores,'filled','MarkerEdgeColor','k'); %EMARKER); %,'Color',ECOLOR,'markersize',EMARKERSIZE); %,'linewidth',EMARKERLINEWIDTH);
    colormap(jet);
    colorbar;
  else
  ...
  %scores=rand(size(labels,1),3); %Temporary placeholder for changing colors of text
  
  for i = 1:size(labels,1)
    hh(i) = text(double(y(i)+0.01),double(x(i)),...
        ELECTRODE_HEIGHT,labels(i,:),'HorizontalAlignment','left',...
	'VerticalAlignment','middle','Color', [1-scores(i) 1-scores(i) 1-scores(i)],'userdata', num2str(allchansind(i)), ...
	'FontSize',EFSIZE, 'buttondownfcn', ...
	    ['tmpstr = get(gco, ''userdata'');'...
	     'set(gco, ''userdata'', get(gco, ''string''));' ...
	     'set(gco, ''string'', tmpstr); clear tmpstr;'] );
  end
  %text explaining label colors and color bar
  text(-0.75,0.4,'NOTE: Higher (red) values and darker');
  text(-0.75,0.35,'labels indicate worse channels'); 
%
