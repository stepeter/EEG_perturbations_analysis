...
% FORMAT   :
%
% EEG = pop_epochbin(EEG, trange, blc, numInds);
%
%INPUTS    :
%
...
% numInds   - event indices to epoch (if want to epoch part of the data) SP 4/14/16
%
% OUTPUTS  :
%
...
function [EEG, com] = mod_pop_epochbin(EEG, trange, blc, numInds, varargin)
com = '';
if nargin < 1
        help pop_epochbin
        return
...
if nargin==1 % using GUI
        ...
        % Somersault
        %
        [EEG, com] = mod_pop_epochbin(EEG, rangtimems, blcorr, 'Warning', 'on', 'History', 'gui');
        return
end
...
p.addRequired('blc');
p.addRequired('numInds', @isnumeric);
% option(s)
p.addParamValue('Warning', 'off', @ischar);
p.addParamValue('History', 'script', @ischar); % history from scripting

p.parse(EEG, trange, blc, numInds, varargin{:});
...
% Epoching
%
EEG = pop_epoch( EEG, bintype, rangtime, 'newname', EEG.setname, 'epochinfo', 'yes','eventindices',numInds);
