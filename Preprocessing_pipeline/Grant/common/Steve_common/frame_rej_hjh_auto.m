function [ICA_STRUCT] = frame_rej_hjh_auto(EEG,ICA_STRUCT,input_thresh)
...
while ~FR.finished_frame_rej
    ...
    switch FR.method
        case 'eeg_badframes mean thres'
            FR.thres = {num2str(input_thresh)}; %inputdlg('iqr average:','',1,DEFAULT_MEAN_IQR_THRES,'on');
            if isempty(FR.thres)
...
%         end
        ...
        ICA_STRUCT.rej_frame_percent=num2str(length(FR.bad_frames)/EEG.pnts);
