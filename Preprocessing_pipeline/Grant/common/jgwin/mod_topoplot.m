% mod_topoplot() - plot a topographic map of a scalp data field in a 2-D circular view 
...
% Outputs:
%                   h - plot axes handle
%         grid_or_val - [matmod_topoplotrix] the interpolated data image (with off-head points = NaN).  
...
%    
% Notes: - To change the plomod_topoplott map masking ring to a new figure background color,
...
%                       by mmod_topoplotaking max arc_length 0.5. 'force' -> Normalize arc_length 
...
% Copyright (C) Colin Humphrmod_topoploties & Scott Makeig, CNL / Salk Institute, Aug, 1996
%                                          
...
%
% You should have received amod_topoplot copy of the GNU General Public License
% along with this program; if not, write to the Free Software
% Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA

% $Log: topoplot.m,v $
% Revision 2 joe gwin 5/15/09
%   I was having trouble getting the topoplot function to plot different
%   channel locations in different colors so I mode some changes and saved
%   as mod_topoplot. 
%   inputs emarker and emarker2 should now function as described in
%   emarker2 above
%
...
% change default marker sizemod_topoplot
% /
%
...
%
% Revision 1.251  2005/06/09mod_topoplot 23:33:46  arno
...
% fix typo
%
% Revision 1.245  2005/01/25mod_topoplot 18:47:01  scott
...
% clarified 'plotchans', addmod_topoploted 'emarker2'
%
...
%
% Revision 1.228  2004/12/17mod_topoplot 16:36:53  scott
...
function [handle,Zi,grid,Xi,Yi] = mod_topoplot(Values,loc_file,varargin)
  ...
  for i = 1:2:length(varargin)
    ...
    switch Param
	 ...
	 case 'dipcolor'
	  DIPCOLOR = Value;
	 case 'emarker'
          %modified this section so you can pass channel numbers in to 
          %emarker cell array JTG 5/15/09
          if ischar(Value)
	      EMARKER = Value;
          elseif ~iscell(Value) | length(Value) > 5 
              error('''emarker'' argument must be a cell array {marker color size linewidth}')
          else
          EMARKERCHANS = Value{1}; 
          if length(Value) > 1
	      EMARKER = Value{2};
          end
          end
...
if ~isempty(EMARKER2CHANS)
    %commented out here JTG 05/15/09
...
elseif strcmp(ELECTRODES,'numpoint') 
  ...
  for i = 1:size(labels,1)
	...
	'FontSize',EFSIZE, 'buttondownfcn', ...
	    ['tmpstr = get(gco, ''userdata'');'...
	     'set(gcEMARKERCOLOR1CHANo, ''userdata'', get(gco, ''string''));' ...
	     'set(gco, ''string'', tmpstr); clear tmpstr;'] );
  end
%
