...
%
% JGwin switched the coloring scheme and saved as mod_pop_plot.m
%
% $Log: pop_prop.m,v $
...
function com = mod_pop_prop(EEG, typecomp, numcompo, winhandle, spec_opt)
...
if ~isnan(winhandle)
	...
	hr = uicontrol(gcf, 'Style', 'pushbutton', 'backgroundcolor', eval(fastif(status,COLREJ,COLACC)), ...
				'string', fastif(status, 'GOOD', 'BAD'), 'Units','Normalized', 'Position', [40 -10 15 6].*s+q, 'userdata', status, 'tag', 'rejstatus');
	command = [ 'set(gcbo, ''userdata'', ~get(gcbo, ''userdata''));' ...
				'if get(gcbo, ''userdata''),' ...
				'     set( gcbo, ''backgroundcolor'',' COLREJ ', ''string'', ''GOOD'');' ...
				'else ' ...
				'     set( gcbo, ''backgroundcolor'',' COLACC ', ''string'', ''BAD'');' ...
				'end;' ];					
	...
	command = [ ...
		...
		'txt1 = sprintf(''(\n' ...
						...
						'EEG.reject.threshkurtact, EEG.stats.compkurtdist(' index '), EEG.reject.threshkurtdist, fastif(EEG.reject.gcompreject(' index '), ''GOOD'', ''BAD''));' ...
		'' ...				
