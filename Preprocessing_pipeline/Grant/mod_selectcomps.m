% mod_selectcomps() - Display components with button to vizualize their
%                  properties and label them for rejection.
% this is pop_selectcomps with a few revisions by J.Gwin 
%   the buttons are now green if gcompreject == 1 and red if 0
%   also I added an optional input (command_option)
%   if 1 (default) the component buttons call mod_pop_prop
%       with option: 'spectopo_options',{'percent',10}
%   if 0 the component buttons set EEG.reject.gcompreject
% Usage:
...
function [EEG, com] = mod_selectcomps( EEG, compnum, command_option, fig );
...
com = '';
if nargin < 1
	help mod_selectcomps;
	return;
end;	

if nargin < 2
    promptstr = { 'Components to plot:' };
    initstr   = { [mod_selectcomps '1:' int2str(size(EEG.icaweights,1)) ] };
    
    result = inputdlg2(promptstr, 'Reject comp. by map -- mod_selectcomps',1, initstr);
...
if length(compnum) > PLOTPERFIG
    for index = 1:PLOTPERFIG:length(compnum)
        mod_selectcomps(EEG, compnum([index:min(length(compnum),index+PLOTPERFIG-1)]), command_option);
    end;

    com = [ 'mod_selectcomps(' inputname(1) ', ' vararg2str(compnum) ');' ];
    return;
end;
...
if ~exist('fig')
	figure('name', [ 'Reject components by map - mod_selectcomps() (dataset: ' EEG.setname ')'], 'tag', currentfigtag, ...
...
for ri = compnum
	...
	if isempty( button )
        ...
        %%%%%J Gwin modified here
        if command_option
            command = sprintf('mod_pop_prop( %s, 0, %d, %3.15f,{''percent'', 10,''limits'',[0 100 NaN NaN NaN NaN]});', inputname(1), ri, button); %added in limits to 100Hz - SP 4/5/17
		else
            command = sprintf(['EEG.reject.gcompreject(' num2str(ri) ') = '...
                'abs(EEG.reject.gcompreject(' num2str(ri) ')-1); '...
                'set( %3.15f , ''backgroundcolor'', '...
                'eval(''fastif(EEG.reject.gcompreject(' num2str(ri) '), '...
                num2str(COLREJ) ', ' num2str(COLACC) ')''));'], button);
        end
        %%%%%
