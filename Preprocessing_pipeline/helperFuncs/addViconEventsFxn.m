% [EEGout]=addViconEventsFxn(subjnum,group);
...
switch group
    ...
    case 'all'
        csv_sync_ch_idx=4:12; EEGTrialnames={'1T','2T','3T'};
...
for i=1:2
    for j=1:size(ViconSyncInds,2)
        ...
        if totalEEGInds(i,j)~=totalViconInds(i,j)
            notEq(i,j)=1;
        end
    end
end
...
%         Y_left_fall=-(1-notEq(2,i))*Y_left_fall;
        if Y_left_rise<=Y_left_fall
            ...
            EEG.event(1,N+q).latency=sync_Rise_EEG(I_left_rise+inds_Prior(1,i))+...
                (ev_diff_Vicon_Left/Vicon_sample_rate)*EEG.srate;
            EEG.event(1,N+q).type=['Pull_Left_' trials{i}];
        else
            ev_diff_Vicon_Left=ev_Left-ViconSyncInds{2,i}(I_left_fall); %difference btwn sync and event in Vicon
            EEG.event(1,N+q).latency=sync_Fall_EEG(I_left_fall+inds_Prior(2,i))+...
                (ev_diff_Vicon_Left/Vicon_sample_rate)*EEG.srate;
            EEG.event(1,N+q).type=['Pull_Left_' trials{i}];   
        end
...
%         Y_right_fall=-(1-notEq(2,i))*Y_right_fall;
        if Y_right_rise<=Y_right_fall
            ...
            EEG.event(1,N+q).latency=sync_Rise_EEG(I_right_rise+inds_Prior(1,i))+...
                (ev_diff_Vicon_Right/Vicon_sample_rate)*EEG.srate;
            EEG.event(1,N+q).type=['Pull_Right_' trials{i}];
        else
            ev_diff_Vicon_Right=ev_Right-ViconSyncInds{2,i}(I_right_fall); %difference btwn sync and event in Vicon
            EEG.event(1,N+q).latency=sync_Fall_EEG(I_right_fall+inds_Prior(2,i))+...
                (ev_diff_Vicon_Right/Vicon_sample_rate)*EEG.srate;
            EEG.event(1,N+q).type=['Pull_Right_' trials{i}];
        end
        if EEG.event(1,N+q).latency<1 || EEG.event(1,N+q).latency>EEG.pnts
...
trialsCompressed=EEGTrialnames; %unique(trials);
