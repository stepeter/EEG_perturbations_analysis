function epochDIPFITData(trigs,pathname1,subjnum,group,trigPrefix,analyzeMismDirection,epochTimes,trigPrefix1,trigPrefix2,savePathSuffix)
...
mergeEpch_path = [pathname1 'STUDY_sets_' group savePathSuffix]; %'withDirection'];
cd(subj_basepath)
...
if analyzeMismDirection==0
    ...
    for i=1:length({EEG.event(1:end).type})
        ccwEv=regexp(EEG.event(1,i).type, regexptranslate('wildcard',[trigPrefix2 '*']),'once'); %[trigPrefix 'CCW_*']),'once');
        cwEv=regexp(EEG.event(1,i).type, regexptranslate('wildcard',[trigPrefix1 '*']),'once'); %[trigPrefix 'CW_*']),'once');
        if ~isempty(ccwEv)
