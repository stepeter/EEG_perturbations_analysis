function [numEpchs]=epochDIPFITData_gaitEvents_onBeam(EEG,pathname,subjnum,group,analyzeFootDirection,epochTimes,twSeq,condsTW,folderSuffix)
...
%     timewarp1.epochs(inds2Rem)=[];
    ...
    %event not at 0
    if ~isempty(timewarp1.latencies)
    ...
    %Check for any latencies outside of true epoch boundaries or where 1st
    %event not at 0
    if ~isempty(timewarp2.latencies)
