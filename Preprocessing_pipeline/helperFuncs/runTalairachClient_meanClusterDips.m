...
clusterNums=3:length(STUDY.cluster); %select clusters to look at (can usually do up to 6 at a time)
outpath=[STUDY.filepath]; %folder to put output in

fid=fopen([outpath filesep STUDY.filename(1:end-6) '_meanClusters.txt'],'w');
for clusternum=clusterNums
    disp(clusternum);
    tal_coords=mni2tal(STUDY.cluster(1,clusternum).dipole.posxyz); %add in mean dipole

    %Now write them out to a file
    fprintf(fid,'%g\t %g\t %g\n',tal_coords(1,1),tal_coords(1,2),tal_coords(1,3));
end
fclose(fid);
