function []=saveAMICAparamfile_jobArray(ICA_path,pFlux,filename,num_chans,num_frames,doPCA,numPCs,maxthreads,group,groupnum)
%Creates param file for AMICA given inputs
%doPCA: 1 - do PCA down to numPCs before ICA; 0 - no PCA
fid = fopen([ICA_path filesep group '_amica' groupnum '.param'],'w');
fprintf(fid,['files ' pFlux filename '\n']);
fprintf(fid,['outdir ' pFlux 'files_AMICA' group groupnum '/\n']);
