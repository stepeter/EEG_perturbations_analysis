if ~exist('ALLCOM')
    addpath(genpath('/usr/local/VR_connectivity/Code/EEG/'));
    startUpEEGLAB('close');
end
close all;


for subjnum=1:2 %[1:13 15:17 19:20 22:33]
    %Set paths
    ...
    EEGsets_outpath = [subj_basepath 'EEG_sets_ASRtest']; % Where you want to save your .set files
%     ICA_path = [subj_basepath 'ICA_Stuff'];
    ...
    EEG=pop_loadset('filename', ['Merge_' group '.set'], 'filepath', ['/usr/local/VR_connectivity/Data/' subjcode filesep 'EEG_sets']);    
    load(['/usr/local/VR_connectivity/Data/' subjcode filesep 'ICA_Stuff' filesep 'files_AMICA' group filesep group '_ICA_PREP_1.mat']);
